...
import { ArrowUp, ArrowRight, ArrowDown, ArrowLeft, RotateCcw, Download, CheckCircle, AlertTriangle, Activity, BrainCircuit, Mic } from 'lucide-react';
    ...
    return (
        <div className="bg-white max-w-4xl mx-auto p-4 sm:p-8 rounded-2xl shadow-2xl animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="bg-gray-50 rounded-xl p-6">
                    ...
                    <div className="mt-4 flex items-center justify-center gap-2">
                        ...
                        <div className={`px-3 py-1 text-xs font-semibold rounded-full ${statusStyles[report.status]}`}>{report.status === 'RELIABLE' ? t('reliable') : t('needs_review')}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
                ...
                <button className="flex-1 flex items-center justify-center gap-2 bg-gray-700 text-white py-3 rounded-lg font-semibold hover:bg-gray-800 transition-colors cursor-not-allowed" disabled><Download size={18}/>{t('export_pdf')}</button>
            </div>
        </div>
    );
};
...
const TestScreen: React.FC<{ questions: any[]; currentQuestion: number; handleAnswer: (rotation: number) => void; }> = ({ questions, currentQuestion, handleAnswer }) => {
  ...
  const commands = useMemo(() => language === 'vi'
    ? [
        { keywords: ['lên', 'trên'], callback: () => handleAnswer(0) },
        { keywords: ['phải'], callback: () => handleAnswer(90) },
        { keywords: ['xuống', 'dưới'], callback: () => handleAnswer(180) },
        { keywords: ['trái'], callback: () => handleAnswer(270) }
      ]
    : [
        { keywords: ['up'], callback: () => handleAnswer(0) },
        { keywords: ['right'], callback: () => handleAnswer(90) },
        { keywords: ['down'], callback: () => handleAnswer(180) },
        { keywords: ['left'], callback: () => handleAnswer(270) }
      ],
  [language, handleAnswer]);
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-2xl mx-auto">
      ...
      {isVoiceControlEnabled && (
         <div className="mt-6 flex items-center justify-center gap-2 text-lg text-blue-600 bg-blue-50 px-4 py-2 rounded-full">
