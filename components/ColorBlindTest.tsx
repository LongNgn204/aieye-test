...
import { RotateCcw, CheckCircle, Activity, BrainCircuit, Mic } from 'lucide-react';
...
const TestScreen: React.FC<{ plates: Plate[]; currentPlate: number; handleSubmit: (answer: string) => void; }> = ({ plates, currentPlate, handleSubmit }) => {
  ...
  const handleVoiceCommand = (transcript: string) => {
    const nothingKeywords = language === 'vi' ? ['không', 'thấy gì', 'không có'] : ['nothing', 'none', "don't see"];
    
    if (nothingKeywords.some(kw => transcript.includes(kw))) {
    ...
    if (numbers) {
        handleSubmit(numbers.join(''));
    }
  };
  
  const { isListening } = useVoiceControl({ commands: [], onNoMatch: handleVoiceCommand });
